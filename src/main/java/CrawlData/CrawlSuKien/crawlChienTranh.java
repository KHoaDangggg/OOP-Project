...
public class crawlChienTranh {
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < list_href.size(); i++) {
            ...
            ArrayList<String> diaDiemLienQuan = new ArrayList<>();
            ...
            for(Element link_tag: link_tags){
                if(link_tag.attr("href").contains("/dia-danh/")){
                    if(!diaDiemLienQuan.contains(link_tag.text())){
                        diaDiemLienQuan.add(link_tag.text());
                    }
                }
            ...
            for (int j = 0; j < data.size(); j++) {
                ...
                obj.setNameRelativePlace(diaDiemLienQuan);
       ...
       JSONArray jsonArray = new JSONArray();
        for(SuKienChienTranh event: list_events){
            ...
            eventJson.put("nameRelativePlace",event.getNameRelativePlace());
            jsonArray.put(eventJson);
        }
        try{
            FileWriter fileWriter = new FileWriter("CT.json");
